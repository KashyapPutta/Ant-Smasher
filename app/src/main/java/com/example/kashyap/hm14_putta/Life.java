...
public class Life {
    ...
    public void birth (Canvas canvas) {
        ...
        if (state == BugState.Dead) {
            ...
            timeToBirth = (float) Math.random () * 30;
        ...
        // Check if bug is alive yet
            ...
            if (curTime - startBirthTimer >= timeToBirth) {
                ...
                if (x < Assets.lifeup.getWidth()/2)
                    x = Assets.lifeup.getWidth()/2;
                else if (x > canvas.getWidth() - Assets.lifeup.getWidth()/2)
                    x = canvas.getWidth() - Assets.lifeup.getWidth()/2;
                y = 0;
    ...
    public void move (Canvas canvas) {
        ...
        if (state == BugState.Alive) {
            ...
            y += (speed * elapsedTime);
            // Draw bug on screen
            canvas.drawBitmap(Assets.lifeup, x,  y, null);
    ...
    public boolean touched (Canvas canvas, int touchx, int touchy) {
        ...
        if (state == BugState.Alive) {
            ...
            if (dis <= Assets.lifeup.getWidth()*1.2f) {
    ...
    public void drawDead (Canvas canvas) {
        if (state == BugState.DrawDead) {
            //canvas.drawBitmap(Assets.lifeclick, x,  y, null);
